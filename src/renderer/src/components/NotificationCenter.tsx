...
export const NotificationCenter = (): React.ReactElement => {
    ...
    useEffect(() => {
        ...
        const setupRealtime = async () => {
            ...
            channel = supabase.channel('notification_center')
                // New DMs for me
                .on(
                    'postgres_changes',
                    { event: 'INSERT', schema: 'public', table: 'direct_messages', filter: `receiver_id=eq.${user.id}` },
                    () => { console.log('New DM!'); fetchNotifications() }
                )
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="absolute right-0 top-full mt-2 w-80 bg-[#f0f0f0] border-2 border-white shadow-[2px_2px_0px_rgba(0,0,0,0.2)] z-50 origin-top-right font-tahoma text-xs"
                    >
                                    ...
                                    <AnimatePresence mode='popLayout'>
                                        {visibleNotifications.map((notif) => (
                                            <motion.div
                                                ...
                                                className={`p-3 border-b border-gray-100 cursor-pointer group transition-colors relative ${seenIds.includes(notif.id) ? 'bg-white hover:bg-gray-50' : 'bg-blue-50 hover:bg-blue-100'
                                                    }`}
                                            >
                                                {/* Dismiss Button */}
                                                <button
                                                ...
                                                <div className="flex items-start gap-3 pr-4">
                                                    ...
                                                    <div className="flex-1">
                                                        <div className="flex justify-between items-start mb-0.5">
                                                            <span className={`font-bold ${seenIds.includes(notif.id) ? 'text-gray-600' : 'text-gray-900'}`}>{notif.title}</span>
                                                            <span className="text-[9px] text-gray-400 whitespace-nowrap ml-2">
                                                                {notif.time}
                                                            </span>
                                                        </div>
                                                        <p className={`${seenIds.includes(notif.id) ? 'text-gray-500' : 'text-gray-700'} line-clamp-2 leading-tight`}>{notif.message}</p>
